...
function checkSystemMessage() {
    ...
    if (sysTitle && sysMsg) {
        Swal.fire({
            ...
            if (currentEmail) {
                client.from('chekiraniya').insert([{ "Име": currentOperator, "Имейл": currentEmail, "Действие": "📢 Прочетено", "Време": new Date().toISOString(), "Локация": "Системно", "Бележка": sysTitle }]).then(res => {
